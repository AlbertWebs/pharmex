    ...
    <div class="left-sidebar">
        ...
        <div class="sidebar-user-pro media border-end">
            <div class="position-relative mx-auto">
                <img src="{{ asset('admin/assets/images/users/user-1.jpg') }}" alt="user"
    ...
    <div class="page-wrapper">
        ...
        <div class="page-content-tab">
                                    ...
                                    <table class="table table-bordered">
                                        ...
                                        <tbody>
                                            @foreach ($Orders as $orders)
                                                <?php $OrderProducts = DB::table('orders_product')
                                                    ->where('orders_id', $orders->id)
                                                    ->get(); ?>
                                                @foreach ($OrderProducts as $products)
                                                    <?php $Products = \App\Models\Product::where('id', $products->orders_product)->get(); ?>
                                                    @if ($Products->isEmpty())
                                                    @else
                                                            ...
                                                            <tr>
                                                                <td>
                                                                    <p class="d-inline-block align-middle mb-0">
                                                                        <a href=""
                                                                            class="d-inline-block align-middle mb-0 product-name fw-semibold">Brand
                                                                            Name:{{ $Product->brand_name }}</a>
                                                                        <br>
                                                                        <a href=""
                                                                            class="d-inline-block align-middle mb-0 product-name fw-semibold">Generic
                                                                            Name:{{ $Product->generic_name }}</a>
                                                                        <br>
                                                                        <span
                                                                            class="text-muted font-13 fw-semibold">{{ $Product->meta }}</span>
                                                                    </p>
                                                                </td>

                                                                <td>{{ $Product->price }}</td>
                                                                <td><span
                                                                        class="">{{ $products->qty * $Product->price }}</span>
                                                                </td>
                                                                <td>
                                                                    {{ $products->qty }}
                                                                </td>
                                                                <td>
                                                                    @if ($orders->status == 'pending')
                                                                        <span
                                                                            class="badge badge-soft-pink">{{ $orders->status }}</span>
                                                                    @elseif($orders->status == 'Accepted')
                                                                        <span
                                                                            class="badge badge-soft-purple">{{ $orders->status }}</span>
                                                                    @else
                                                                        <span
                                                                            class="badge badge-success alert-success">{{ $orders->status }}</span>
                                                                    @endif
                                                                </td>
            ...
            <footer class="footer text-center text-sm-start">
                ...
                </script> Pharmex <span class="text-muted d-none d-sm-inline-block float-end">MVP
                    Crafted with <i class="mdi mdi-heart text-danger"></i> by Drenla Hub</span>
            </footer>
            <!-- end Footer -->
            <!--end footer-->
        </div>
